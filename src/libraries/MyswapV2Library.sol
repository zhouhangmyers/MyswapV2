...
library MyswapV2Library {
    function sortTokens(address tokenA, address tokenB) internal pure returns (address token0, address token1) {
        ...
        require(token0 != address(0), "MyswapV2Library: ZERO_ADDRESS");
    }
                ...
                uint256(
                    keccak256(
                        abi.encodePacked(
                            ...
                            keccak256(type(MyswapV2Pair).creationCode)
                        )
                    )
                )
            )
        ...
        returns (uint256 reserveA, uint256 reserveB)
    {
        ...
        (uint256 reserve0, uint256 reserve1,) = MyswapV2Pair(pairFor(factory, tokenA, tokenB)).getReserves();
        (reserveA, reserveB) = tokenA == token0 ? (reserve0, reserve1) : (reserve1, reserve0);
    }

    function quote(uint256 amountA, uint256 reserveA, uint256 reserveB) internal pure returns (uint256 amountB) {
        require(amountA > 0, "MyswapV2Library: INSUFFICIENT_AMOUNT");
        require(reserveA > 0 && reserveB > 0, "MyswapV2Library: INSUFFICIENT_LIQUIDITY");
    ...
    {
        require(amountIn > 0, "MyswapV2Library: INSUFFICIENT_INPUT_AMOUNT");
